  ...
  return (
    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden animate-in zoom-in duration-500">
       <div className="p-5 md:p-6 bg-primary text-white flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3"><Users size={20} /><h3 className="text-xs md:text-sm font-semibold uppercase tracking-wider">{t('managementTeams')}</h3></div>
          <button onClick={onAdd} className="w-full sm:w-auto px-6 py-2 bg-white/10 rounded-xl text-[10px] font-semibold uppercase tracking-widest flex items-center justify-center gap-2"><Plus size={14}/> {t('createTeam')}</button>
       </div>
       <div className="p-4 md:p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          ...
          {teams.length === 0 && (
            <div className="col-span-full py-20 text-center space-y-4">
              <p className="text-slate-400 font-semibold uppercase text-xs tracking-widest">{t('noTeams')}</p>
            </div>
          )}
       </div>
    </div>
