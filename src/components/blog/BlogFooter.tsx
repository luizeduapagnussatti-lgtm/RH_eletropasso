...
const BlogFooter: React.FC = () => {
  ...
  const goToBlog = () => navigateTo('/blog');
  const goToTutorials = () => navigateTo('/how-to-use');

  const columns = [
    {
      title: t('footer.product'),
      links: [
        { label: t('home'), action: goHome },
        { label: t('features'), action: () => { navigateTo('/features'); } },
        { label: t('blog'), action: goToBlog },
        { label: t('changelog'), action: () => { navigateTo('/changelog'); } },
      ],
    },
    {
      title: t('footer.resources'),
      links: [
        { label: t('guides'), action: goToTutorials },
        { label: t('github'), action: () => window.open('https://github.com/mimnets/openhrapp', '_blank') },
      ],
    },
    {
      title: t('footer.company'),
      links: [
        { label: t('about'), action: () => { navigateTo('/about'); } },
        { label: t('nav.contact'), action: goHome },
        { label: t('privacy'), action: () => { navigateTo('/privacy'); } },
        { label: t('terms'), action: () => { navigateTo('/terms'); } },
      ],
    },
  ];
          ...
          {columns.map((col) => (
            <div key={col.title}>
              ...
              <ul className="space-y-3">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <button onClick={link.action} className="text-sm text-slate-400 hover:text-white transition-colors">
                      {link.label}
                    </button>
                  </li>
                ))}
